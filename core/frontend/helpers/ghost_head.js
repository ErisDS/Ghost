...
const {metaData, escapeExpression, SafeString, logging, settingsCache, config, blogIcon} = require('../services/proxy');
...
function getMembersHelper(data) {
    ...
    let membersHelper = `<script defer src="${portalUrl}" data-ghost="${config.getSiteUrl()}"${colorString}></script>`;
