...
describe.only('templates', function () {
    ...
    describe('[private] getTemplateForSingle', function () {
        beforeEach(function () {
            hasTemplateStub = sandbox.stub().returns(false);
        ...
        describe('with many templates', function () {
            ...
            it('post without custom slug template', function () {
                var view = _private.getTemplateForSingle({
                    page: 0,
                    slug: 'test-post'
                });
                should.exist(view);
        ...
        it('will fall back to post even if no index.hbs', function () {
            ...
            var view = _private.getTemplateForSingle({page: 1});
            should.exist(view);
            view.should.eql('post');
        });
    });
    ...
    describe('setTemplate', function () {
        ...
        beforeEach(function () {
            ...
            stubs.getTemplateForSingle = sandbox.stub(_private, 'getTemplateForSingle').returns('testFromSingle');
        ...
        it('does nothing if template is already set', function () {
            // Pre-set template
            res.locals.template = 'thing';

            // Call setTemplate
            templates.setTemplate(req, res);

            // It hasn't changed
            res.locals.template.should.eql('thing');

            // And nothing got called
            stubs.pickTemplate.called.should.be.false();
            stubs.getTemplateForSingle.called.should.be.false();
        ...
        it('defaults to index', function () {
            ...
            // It should be index
            res.locals.template.should.eql('index');
        ...
        it('calls pickTemplate for custom routes', function () {
            ...
            res.locals.template.should.eql('testFromPickTemplate');
        ...
        it('calls getTemplateForSingle for single routes', function () {
            res.locals.route = {
                type: 'single'
            };

            // Requires a post to be set
            res.data = {post: {slug: 'test'}};

            // Call setTemplate
            templates.setTemplate(req, res);

            // should be testFromSingle
            res.locals.template.should.eql('testFromSingle');

            // Only pickTemplate got called
            stubs.pickTemplate.called.should.be.false();
            stubs.getTemplateForSingle.called.should.be.true();
            stubs.getTemplateForChannel.called.should.be.false();
            stubs.getTemplateForError.called.should.be.false();

            stubs.getTemplateForSingle.calledWith({slug: 'test'}).should.be.true();
        });
        ...
        it('calls getTemplateForChannel for channel routes', function () {
            ...
            res.locals.template.should.eql('testFromChannel');
        ...
        it('calls getTemplateForError if there is an error', function () {
            ...
            res.locals.template.should.eql('testFromError');
