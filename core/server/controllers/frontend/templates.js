 ...
 * @returns {String[]}
 */
_private.getSingleTemplateHierarchy = function getSingleTemplateHierarchy(postObject) {
...
_private.getTemplateForSingle = function getTemplateForSingle(postObject) {
    var templateList = _private.getSingleTemplateHierarchy(postObject),
...
module.exports.setTemplate = function setTemplate(req, res) {
    var routeConfig = res.locals.route || {};
    ...
    if (req.err) {
        res.locals.template = _private.getTemplateForError(res.statusCode);
        return;
    }

    switch (routeConfig.type) {
        case 'custom':
            res.locals.template = _private.pickTemplate(routeConfig.templateName, routeConfig.defaultTemplate);
            break;
        case 'channel':
            res.locals.template = _private.getTemplateForChannel(res.locals.channel);
            break;
        case 'single':
            res.locals.template = _private.getTemplateForSingle(res.data.post);
            break;
        default:
