...
describe('Automation email analytics', function () {
  ...
  beforeAll(async function () {
    ...
    const emailDesignSetting = await testUtils
      .knex('email_design_settings')
      .where('slug', DEFAULT_EMAIL_DESIGN_SETTING_SLUG)
      .first('id');
    assert.ok(
      emailDesignSetting,
      `expected the ${DEFAULT_EMAIL_DESIGN_SETTING_SLUG} design setting to exist`,
    );
  ...
  it('records delivered events against the automated email recipient', async function () {
    ...
    assertDateEqual(
      ...
      EVENT_DATE,
      'delivered_at should match the Mailgun event timestamp',
    );
  ...
  it('records opened events and counts the open against the revision', async function () {
    ...
    assertDateEqual(
      updated.opened_at,
      EVENT_DATE,
      'opened_at should match the Mailgun event timestamp',
    );
  ...
  it('keeps the earliest timestamp when a message has several events', async function () {
    ...
    const updatedRevision = await readRevision(revision.id);
    assert.equal(
      updatedRevision.email_opened_count,
      1,
      'a recipient that opens twice is still one opener',
    );
  });

  it('counts every recipient of a revision that opens', async function () {
    const revision = await createSendEmailRevision();
    const first = await recordEmailSent({
      revision,
      mailgunMessageId: 'opener-one@mg.example.com',
    });
    const second = await recordEmailSent({
      revision,
      mailgunMessageId: 'opener-two@mg.example.com',
    });

    mailgunEvents = [
      mailgunEvent({
        type: 'opened',
        messageId: 'opener-one@mg.example.com',
        recipientEmail: first.member_email,
      }),
      mailgunEvent({
        type: 'opened',
        messageId: 'opener-two@mg.example.com',
        recipientEmail: second.member_email,
      }),
    ];
  ...
  it('does not count an open again when a later job refetches it', async function () {
    ...
    assert.equal(
      (await readRevision(revision.id)).email_opened_count,
      1,
      'the open should only count once',
    );
    assertDateEqual(
      (await readRecipient(recipient.id)).opened_at,
      EVENT_DATE,
  ...
  it('routes events to the right recipient and ignores unknown message ids', async function () {
    ...
    const second = await recordEmailSent({
      ...
      mailgunMessageId: 'second@mg.example.com',
    });

    mailgunEvents = [
      mailgunEvent({
        type: 'delivered',
        messageId: 'first@mg.example.com',
        recipientEmail: first.member_email,
      }),
      mailgunEvent({
        type: 'delivered',
        messageId: 'unknown@mg.example.com',
    ...
    assertDateEqual(
      updatedFirst.delivered_at,
      EVENT_DATE,
      'the matching recipient should be marked delivered',
    );
