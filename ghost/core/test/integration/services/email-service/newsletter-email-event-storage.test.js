...
processingModes.forEach(({ name, batchProcessing }) => {
  describe(`NewsletterEmailEventStorage - ${name}`, function () {
    ...
    it('Can handle delivered events', async function () {
      ...
      assert.equal(
        updatedEmailRecipient.get('delivered_at').toUTCString(),
    ...
    it('Can handle permanent failure events', async function () {
      ...
      assert.equal(
        permanentFailures.models[0].get('message'),
      ...
      assert.equal(
        permanentFailures.models[0].get('failed_at').toUTCString(),
        timestamp.toUTCString(),
      );
    });
    ...
    it('Ignores permanent failures if already failed', async function () {
      ...
      assert.notEqual(
        initialModel.get('failed_at'),
        null,
        'This test requires a failed email recipient',
      );
      ...
      assert.equal(
        updatedEmailRecipient.get('failed_at').toUTCString(),
        initialModel.get('failed_at').toUTCString(),
      );
    ...
    it('Can handle complaint events', async function () {
      ...
      const {
        body: { events: eventsBefore },
      } = await agent.get(eventsURI);
      const existingSpamEvent = eventsBefore.find(
        (event) => event.type === 'email_complaint_event',
      );
      assert.equal(
        existingSpamEvent,
        undefined,
        'This test requires a member that does not have a spam event',
      );
      ...
      const {
        body: { events: eventsAfter },
      } = await agent.get(eventsURI);
      const spamComplaintEvent = eventsAfter.find(
        (event) => event.type === 'email_complaint_event',
      );
    ...
    it('Can handle unsubscribe events', async function () {
      ...
      const memberInitial = await membersService.api.members.get(
        { id: memberId },
        { withRelated: ['newsletters'] },
      );
      assert.equal(
        memberInitial.related('newsletters').length,
        2,
        'This test requires a member that is subscribed to at least one newsletter',
      );
      ...
      assert(
        !member
          .related('newsletters')
          .models.some((newsletter) => newsletter.id === newsletterToRemove),
      );
      ...
      assert(
        member
          .related('newsletters')
          .models.some((newsletter) => newsletter.id === newsletterToKeep),
      );
    });
    ...
    it('Keeps other newsletters when the member changed their email after the send', async function () {
      ...
      await membersService.api.members.update(
        { newsletters: [{ id: newsletterToRemove }, { id: newsletterToKeep }] },
        { id: memberId },
      );
      ...
      assert.equal(
        memberInitial.related('newsletters').length,
        2,
        'precondition: subscribed to 2 newsletters',
      );
      ...
      assert.equal(
        member.related('newsletters').length,
        1,
