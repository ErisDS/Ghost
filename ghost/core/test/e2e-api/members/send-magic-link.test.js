...
describe('sendMagicLink', function () {
  ...
  describe('signin email', function () {
    ...
    function scrubEmailContent(mail) {
      const scrub = (s) =>
        s && s.replace(/([?&])token=[^&\s'\"]+/gi, '$1token=<TOKEN>').replace(/\d{6}/g, '<OTC>');
    ...
    async function sendSigninRequest(options = {}) {
      ...
      await postWithIntegrityToken('/api/send-magic-link').body(body).expectStatus(201);
  ...
  describe('OTC', function () {
    function sendMagicLinkRequest(email, emailType = 'signin', otc = false) {
      ...
      return postWithIntegrityToken('/api/send-magic-link').body(body);
    }
    ...
    function assertNoOTCInEmailContent(mail) {
      ...
      assert(
        !subjectMatch,
        `Email subject should not contain OTC. Found: "${subjectMatch?.[0]}" in subject: "${mail.subject}"`,
      );
      ...
      assert(
        !textMatch,
        `Email text should not contain OTC. Found: "${textMatch?.[0]}" near: "${text.substring(text.search(otcRegex) - 50, text.search(otcRegex) + 100)}"`,
      );
      ...
      assert(
        !htmlMatch,
        `Email HTML should not contain OTC. Found: "${htmlMatch?.[0]}" near: "${htmlText.substring(htmlText.search(otcRegex) - 50, htmlText.search(otcRegex) + 100)}"`,
      );
    }
    ...
    it('Should return empty body for signin magic link requests', async function () {
      await sendMagicLinkRequest('member1@test.com').expectEmptyBody().expectStatus(201);
    });

    it('Should include otc_ref in response when requesting magic link with OTC', async function () {
      const response = await sendMagicLinkRequest('member1@test.com', 'signin', true).expectStatus(
        201,
      );
    ...
    it('Should not include otc_ref in response when requesting magic link without OTC', async function () {
      const response = await sendMagicLinkRequest('member1@test.com', 'signin', false).expectStatus(
        201,
      );
    ...
    [true, 'true'].forEach((otcValue) => {
      it(`Should include OTC when requested with otc parameter value: ${otcValue}`, async function () {
        const response = await sendMagicLinkRequest(
          'member1@test.com',
          'signin',
          otcValue,
        ).expectStatus(201);
    ...
    [false, 'false'].forEach((otcValue) => {
      it(`Should not include OTC when requested with otc parameter value: ${otcValue}`, async function () {
        ...
        assert(
          !response.body.otc_ref,
          `Response should not contain otc_ref for includeOTC=${otcValue}`,
        );
    ...
    it('Should gracefully handle OTC generation failures', async function () {
      ...
      const deriveOTCStub = sinon
        .stub(tokenProvider.prototype, 'deriveOTC')
        .throws(new Error('OTC generation failed'));
      ...
      try {
        ...
        assert(
          !response.body.otc_ref,
          'Response should not contain otc_ref when OTC generation fails',
        );
    ...
    async function sendAndVerifyOTC(email, emailType = 'signin', options = {}) {
      const response = await sendMagicLinkRequest(email, emailType, true).expectStatus(201);
    ...
    it('/verify-otc endpoint returns correct redirectUrl using redirect body param', async function () {
      const verifyResponse = await sendAndVerifyOTC('member1@test.com', 'signin', {
        referer: 'https://www.test.com/signin',
        redirect: 'https://www.test.com/post',
      });
    ...
    describe('Rate limiting', function () {
      ...
      it('Will rate limit OTC verification enumeration (IP-based)', async function () {
        const otcVerificationEnumerationLimit =
          configUtils.config.get('spam').otc_verification_enumeration.freeRetries + 1;
      ...
      it('Will rate limit OTC verification for specific otcRef', async function () {
        const otcVerificationLimit =
          configUtils.config.get('spam').otc_verification.freeRetries + 1;
      ...
      it('Different otcRefs are tracked independently', async function () {
        ...
        assert(
          otcVerificationLimit < otcVerificationEnumerationLimit,
          'Specific otcRef limit must be lower than enumeration limit for this test',
        );
