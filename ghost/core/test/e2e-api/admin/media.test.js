...
describe('Media API', function () {
  ...
  describe('media/upload', function () {
    it('Can upload a MP4', async function () {
      ...
      assert.match(
        new URL(res.body.media[0].url).pathname,
        /\/content\/media\/\d+\/\d+\/sample_640x360\.mp4/,
      );
      assert.match(
        new URL(res.body.media[0].thumbnail_url).pathname,
        /\/content\/media\/\d+\/\d+\/sample_640x360_thumb\.png/,
      );
    ...
    it('Can upload a WebM without a thumbnail', async function () {
      ...
      assert.match(
        new URL(res.body.media[0].url).pathname,
        /\/content\/media\/\d+\/\d+\/sample_640x360\.webm/,
      );
    ...
    it('Can upload an Ogg', async function () {
      ...
      assert.match(
        new URL(res.body.media[0].url).pathname,
        /\/content\/media\/\d+\/\d+\/sample_640x360\.ogv/,
      );
    ...
    it('Can upload an Ogg audio with application/ogg content type', async function () {
      const res = await request
        ...
        .attach('file', path.join(__dirname, '/../../utils/fixtures/media/sample.ogg'), {
          filename: 'sample.ogg',
    ...
    it('Can upload an m4a with audio/mp4 content type', async function () {
      const res = await request
        ...
        .attach('file', path.join(__dirname, '/../../utils/fixtures/media/sample.m4a'), {
          filename: 'audio-mp4.m4a',
      ...
      assert.match(
        new URL(res.body.media[0].url).pathname,
        /\/content\/media\/\d+\/\d+\/audio-mp4\.m4a/,
      );
    ...
    it('Can upload an m4a with audio/x-m4a content type', async function () {
      ...
      assert.match(
        new URL(res.body.media[0].url).pathname,
        /\/content\/media\/\d+\/\d+\/audio-x-m4a\.m4a/,
      );
    ...
    it('Passes the content type to the storage adapter when uploading an MP4', async function () {
      ...
      assert.equal(
        saveSpy.callCount,
        2,
        'save() should have been called twice (thumbnail + media)',
      );
      const thumbnailArg = saveSpy.firstCall.args[0];
      assert.equal(
        thumbnailArg.type,
        'image/png',
        'save() should receive the correct content type for the thumbnail',
      );
      const mediaArg = saveSpy.secondCall.args[0];
      assert.equal(
        mediaArg.type,
        'video/mp4',
        'save() should receive the correct content type for the media file',
      );
    });
    ...
    it('Passes the content type to the storage adapter when uploading an MP3', async function () {
      ...
      assert.equal(
        fileArg.type,
        'audio/mpeg',
        'save() should receive the correct content type for audio files',
      );
    });
  ...
  describe('media/thumbnail/upload', function () {
    it('Can update existing thumbnail', async function () {
      ...
      const thumbnailRes = await request
        .put(localUtils.API.getApiQuery(`media/thumbnail/upload`))
        .set('Origin', config.get('url'))
        .expect('Content-Type', /json/)
