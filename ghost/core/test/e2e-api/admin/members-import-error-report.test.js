...
describe('Members import error report', function () {
    ...
    beforeAll(async function () {
        ...
        assert.ok(email.attachments && email.attachments[0], 'the completion email carries an attachment');
    ...
    it('keeps the tiers column present but empty', function () {
        ...
        assert.ok(rows.every(row => row.tiers === ''), 'tiers is empty for every failed row');
    });
