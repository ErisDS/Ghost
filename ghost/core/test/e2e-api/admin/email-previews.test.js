...
describe('Email Preview API', function () {
  ...
  describe('Read', function () {
    it("can't retrieve for non existent post", async function () {
      await agent
        .get('email_previews/posts/abcd1234abcd1234abcd1234/')
        .expectStatus(404)
        .matchHeaderSnapshot({
    ...
    it('can read post email preview with email card and replacements', async function () {
      ...
      const post = testUtils.DataGenerator.forKnex.createPost({
        ...
        mobiledoc:
          '{"version":"0.3.1","atoms":[],"cards":[["email",{"html":"<p>Hey {first_name \\"there\\"} {unknown}</p><p><strong>Welcome to your first Ghost email!</strong></p>"}],["email",{"html":"<p>Another email card with a similar replacement, {first_name, \\"see?\\"}</p>"}]],"markups":[],"sections":[[10,0],[1,"p",[[0,[],0,"This is the actual post content..."]]],[10,1],[1,"p",[]]]}',
    ...
    it('has custom content transformations for email compatibility', async function () {
      ...
      const post = testUtils.DataGenerator.forKnex.createPost({
        ...
        slug: 'email-only-card',
        mobiledoc:
          '{"version":"0.3.1","atoms":[],"cards":[],"markups":[["a",["href","https://ghost.org"]]],"sections":[[1,"p",[[0,[],0,"Testing "],[0,[0],1,"links"],[0,[],0," in email excerpt and apostrophes \'"]]]]}',
        html: '<p>This is the actual post content...</p>',
        plaintext: 'This is the actual post content...',
        status: 'draft',
        uuid: 'd52c42ae-2755-455c-80ec-70b2ec55c904',
    ...
    it('uses the posts newsletter by default', async function () {
      ...
      await agent
        ...
        .expect(({ body }) => {
          ...
          assert.doesNotMatch(
            body.email_previews[0].html,
            new RegExp(defaultNewsletter.get('name')),
          );
  ...
  describe('Tier-restricted posts', function () {
    ...
    beforeAll(async function () {
              ...
              children: [
                {
                  ...
                  mode: 'normal',
                  style: '',
                  text: 'Members only tier content',
                  type: 'text',
                  version: 1,
                },
              ],
      ...
      const { body: draftBody } = await agent
        .post('posts/')
        .body({
          posts: [{ title: 'Tier-restricted email preview post', status: 'draft', lexical }],
        })
        .expectStatus(201);
    ...
    it('renders the full content for the paid member preview', async function () {
      await agent
        ...
        .expect(({ body }) => {
          ...
          assert.ok(
            html.includes('Members only tier content'),
            'paid member preview should include the gated content',
          );
          assert.ok(
            !html.includes('Become a paid member'),
            'paid member preview should not include the paywall CTA',
          );
        });
    });
    ...
    it('renders the paywall for the free member preview', async function () {
      await agent
        ...
        .expect(({ body }) => {
          const { html } = body.email_previews[0];
          assert.ok(
            html.includes('Free preview text'),
            'free member preview should include the public preview',
          );
          assert.ok(
            !html.includes('Members only tier content'),
            'free member preview should not include the gated content',
          );
          assert.ok(
            html.includes('Become a paid member'),
            'free member preview should include the paywall CTA',
          );
        });
    });
    ...
    it('renders content access for the specifically selected tier', async function () {
      await agent
        ...
        .expect(({ body }) => {
          ...
          assert.ok(
            html.includes('Members only tier content'),
            'selected tier preview should include the gated content',
          );
          assert.ok(
            !html.includes('Become a paid member'),
            'selected tier preview should not include the paywall CTA',
          );
        });
    });
    ...
    it('renders the paywall for a specifically selected tier without access', async function () {
      await agent
        ...
        .expect(({ body }) => {
          ...
          assert.ok(
            !html.includes('Members only tier content'),
            'other tier preview should not include the gated content',
          );
          assert.ok(
            html.includes('Become a paid member'),
            'other tier preview should include the paywall CTA',
          );
        });
    });

    it('rejects an invalid member_status', async function () {
      await agent.get(`email_previews/posts/${post.id}/?member_status=comped`).expectStatus(422);
    });
    ...
    it('rejects a repeated member_tier param', async function () {
      ...
      await agent
        .get(
          `email_previews/posts/${post.id}/?member_status=paid&member_tier=${paidTierSlug}&member_tier=other`,
        )
        .expectStatus(422);
    });
    ...
    it('maps a legacy memberSegment onto the paid audience', async function () {
      ...
      await agent
        ...
        .expect(({ body }) => {
          ...
          assert.ok(
            html.includes('Members only tier content'),
            'legacy paid preview should include the gated content',
          );
        });
    });
    ...
    it('maps a legacy memberSegment onto the free audience', async function () {
      await agent
        ...
        .expect(({ body }) => {
          ...
          assert.ok(
            !html.includes('Members only tier content'),
            'legacy free preview should not include the gated content',
          );
          assert.ok(
            html.includes('Become a paid member'),
            'legacy free preview should include the paywall CTA',
          );
        });
    });
