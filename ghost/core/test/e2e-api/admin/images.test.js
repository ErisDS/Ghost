...
const uploadImageRequest = ({ fileContents, filename, contentType, ref }) => {
  ...
  return agent.post('/images/upload/').body(form);
};
...
const uploadImageCheck = async ({
  ...
  assert.match(
    body.images[0].url,
    new RegExp(`${urlUtils.urlFor('home', true)}content/images/\\d+/\\d+/${expectedFileName}`),
  );
  ...
  const originalFilePath = skipOriginal
    ? filePath
    : expectedOriginalFileName
      ? filePath.replace(expectedFileName, expectedOriginalFileName)
      : imageTransform.generateOriginalImageName(filePath);
  ...
  const resizedPath = relativePath.replace(
    '/content/images/',
    '/content/images/size/w10/format/webp/',
  );
  await frontendAgent.get(resizedPath).expect('Content-Type', 'image/webp').expect(200);
  ...
  if (!contentType.includes('svg')) {
    ...
    await frontendAgent.get(resizedPath2).expect('Content-Type', contentType).expect(200);
...
describe('Images API', function () {
  ...
  it('Can upload a png', async function () {
    ...
    await uploadImageCheck({
      path: originalFilePath,
      filename: 'ghost-logo.png',
      contentType: 'image/png',
      ref: 'https://ghost.org/ghost-logo.png',
    });
  });
  ...
  it('Can upload a svg that needs sanitization', async function () {
    ...
    await uploadImageCheck({
      path: originalFilePath,
      filename: 'svg-with-unsafe-script.svg',
      contentType: 'image/svg+xml',
      skipOriginal: true,
    });
  });
  ...
  it('Can upload a zipped svg that needs sanitization', async function () {
    const originalFilePath = p.join(
      __dirname,
      '/../../utils/fixtures/images/svgz-with-unsafe-script.svgz',
    );
    await uploadImageCheck({
      path: originalFilePath,
      filename: 'svg-with-unsafe-script.svgz',
      contentType: 'image/svg+xml',
      skipOriginal: true,
    });
  });
  ...
  it('Can upload a square profile image', async function () {
    const originalFilePath = p.join(
      __dirname,
      '/../../utils/fixtures/images/loadingcat_square.gif',
    );
    await uploadImageCheck({
      path: originalFilePath,
      filename: 'loadingcat_square.gif',
      contentType: 'image/gif',
    });
  });
  ...
  it('Can upload multiple images with the same name', async function () {
    ...
    await uploadImageCheck({
      ...
      filename: 'a.png',
      contentType: 'image/png',
      expectedFileName: 'a-1.png',
      expectedOriginalFileName: 'a-1_o.png',
    });
  });
  ...
  it('Trims _o suffix from uploaded files', async function () {
    ...
    await uploadImageCheck({
      ...
      filename: 'a-3_o.png',
      contentType: 'image/png',
      expectedFileName: 'a-3.png',
      expectedOriginalFileName: 'a-3_o.png',
    });
  });
  ...
  it('Can use _o in uploaded file name, as long as it is not at the end', async function () {
    ...
    await uploadImageCheck({
      ...
      filename: 'a_o-3.png',
      contentType: 'image/png',
      expectedFileName: 'a_o-3.png',
      expectedOriginalFileName: 'a_o-3_o.png',
    });
  });
  ...
  it('Errors when image request body is broken', async function () {
    ...
    const brokenDataBlob = await new Blob(
      [brokenPayload, blob.slice(0, Math.floor(blob.size / 2))],
      {
  ...
  it('Passes the content type to the storage adapter when uploading a GIF', async function () {
    ...
    assert.equal(
      fileArg.type,
      'image/gif',
      'save() should receive the correct content type for image files',
    );
  });
