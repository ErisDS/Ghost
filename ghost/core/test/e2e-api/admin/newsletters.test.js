...
describe('Newsletters API', function () {
  ...
  it('Can include members, active members & posts counts when reading a newsletter', async function () {
    await agent
      .get(
        `newsletters/${fixtureManager.get('newsletters', 0).id}/?include=count.members,count.active_members,count.posts`,
      )
      .expectStatus(200)
      .matchBodySnapshot({
  ...
  it('Can include members, active members & posts counts when editing newsletters', async function () {
    const id = fixtureManager.get('newsletters', 0).id;
    await agent
      .put(`newsletters/${id}/?include=count.members,count.active_members,count.posts`)
      .body({
        newsletters: [
          {
  ...
  describe('Host Settings: newsletter limits', function () {
    ...
    describe('Max limit', function () {
        ...
        await agent
          ...
          .expect(({ body }) => {
            assert.equal(
              ...
              'Your plan supports up to 4 newsletters. Please upgrade to add more.',
            );
          });
      });
      ...
      it("Editing an active newsletter doesn't fail", async function () {
        ...
        assert.ok(
          activeNewsletter,
          'This test expects to have an active newsletter in the test fixtures',
        );
      ...
      it("Editing an archived newsletter doesn't fail", async function () {
        ...
        assert.ok(
          archivedNewsletter,
          'This test expects to have an archived newsletter in the test fixtures',
        );
  ...
  describe('Managed email without custom sending domain', function () {
    ...
    it('Can set newsletter reply-to to any email address with required verification', async function () {
      ...
      assert.equal(
        before.get('sender_reply_to'),
        beforeSenderReplyTo,
        'sender_reply_to should not have changed because it first requires verification',
      );
    ...
    describe('Auto correcting invalid domains', function () {
      ...
      it('Browse returns sender_email as sender_reply_to in case we cannot send from sender_email and sender_reply_to is set to newsletter', async function () {
        const { body } = await agent.get(`newsletters`).expectStatus(200).matchHeaderSnapshot({
          'content-version': anyContentVersion,
          etag: anyEtag,
        });
  ...
  describe('URL transformations', function () {
    ...
    it('Can read newsletter with header_image as absolute site URL', async function () {
      const res = await agent.get('newsletters/?filter=slug:new-newsletter').expectStatus(200);
