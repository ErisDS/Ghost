...
// the adapter-manager is required via its stable service facade (so its methods stay
...
describe('adapter-manager route-settings wiring', function () {
  ...
  it('resolves the bundled default settings path for the S3 store without operator config', function () {
    ...
    const { adapterConfig } = resolveAdapterOptions(
      'route-settings',
      normalizeAdapterConfig(configUtils.config),
    );
