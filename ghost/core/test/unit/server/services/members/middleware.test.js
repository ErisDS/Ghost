...
describe('Members Service Middleware', function () {
  describe('createSessionFromMagicLink', function () {
    ...
    it('calls next if url does not include a token', async function () {
      await request(app).get('/members').expect(204);
    });
    ...
    it('redirects member to referrer param path on signin if it is on the site', async function () {
      ...
      await request(app)
        ...
        .expect(
          'Location',
          'https://site.com/blah/my-post/?action=signin&success=true#comment-123',
        );
    });
    ...
    it('redirects member to referrer param path on signup if it is on the site', async function () {
      ...
      await request(app)
        ...
        .expect(
          'Location',
          'https://site.com/blah/my-post/?action=signup&success=true#comment-123',
        );
    });
    ...
    it('redirects member to referrer param path on failure if it is on the site', async function () {
      ...
      await request(app)
        ...
        .expect(
          'Location',
          'https://site.com/blah/?action=subscribe&success=false#/portal/account?giftRedemption=true',
        );
    });
  ...
  describe('setAccessCookies (via accessInfoSession)', function () {
    ...
    beforeEach(function () {
      ...
      sinon
        .stub(config, 'get')
        .withArgs('cacheMembersContent:hmacSecret')
        .returns(hmacSecret)
        .withArgs('cacheMembersContent:enabled')
        .returns(true);
    });
    ...
    it('uses Path=/ for root site installs', async function () {
      ...
      assert.ok(
        accessCookie.includes('Path=/;'),
        `Expected Path=/ in ghost-access: ${accessCookie}`,
      );
      assert.ok(
        hmacCookie.includes('Path=/;'),
        `Expected Path=/ in ghost-access-hmac: ${hmacCookie}`,
      );
    });
    ...
    it('uses Path=/subdir for subdirectory site installs', async function () {
      ...
      assert.ok(hmacCookie, 'ghost-access-hmac cookie should be set');
      assert.ok(
        accessCookie.includes('Path=/subdir;'),
        `Expected Path=/subdir in ghost-access: ${accessCookie}`,
      );
      assert.ok(
        hmacCookie.includes('Path=/subdir;'),
        `Expected Path=/subdir in ghost-access-hmac: ${hmacCookie}`,
      );
    });
    ...
    it('clears cookies with Path=/ for root site installs', async function () {
      ...
      assert.match(
        accessCookie,
        /^ghost-access=null;.*Path=\/;/,
        `Expected null with Path=/ in ghost-access: ${accessCookie}`,
      );
      assert.match(
        hmacCookie,
        /^ghost-access-hmac=null;.*Path=\/;/,
        `Expected null with Path=/ in ghost-access-hmac: ${hmacCookie}`,
      );
    });
    ...
    it('clears cookies with Path=/subdir for subdirectory site installs', async function () {
      ...
      assert.match(
        accessCookie,
        /^ghost-access=null;.*Path=\/subdir;/,
        `Expected null with Path=/subdir in ghost-access: ${accessCookie}`,
      );
      assert.match(
        hmacCookie,
        /^ghost-access-hmac=null;.*Path=\/subdir;/,
        `Expected null with Path=/subdir in ghost-access-hmac: ${hmacCookie}`,
      );
    });
  });
});
