...
const {
  generateGiftKeyData,
  applyGiftAccess,
} = require('../../../../../core/server/api/endpoints/utils/gift-link-access');
...
describe('Gift link access', function () {
  ...
  for (const [docName, controller] of [
    ['posts', postsPublicController],
    ['pages', pagesPublicController],
  ] as const) {
  ...
  describe('applyGiftAccess', function () {
    ...
    it('refuses to run without the cache-key resolution, so verification can never ship uncoupled from the key', async function () {
      ...
      await assert.rejects(
        applyGiftAccess(frame, { id: 'post-1' }),
