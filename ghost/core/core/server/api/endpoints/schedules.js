...
const controller = {
  ...
  publish: {
    ...
    async query(frame) {
      ...
      const { scheduledResource, preScheduledResource } = await postScheduling.publish(
        resourceType,
        frame.options.id,
        frame.data.force,
        options,
      ...
      setCacheInvalidateHeader(
        frame,
        postScheduling.handleCacheInvalidation(scheduledResource, preScheduledResource),
      );
